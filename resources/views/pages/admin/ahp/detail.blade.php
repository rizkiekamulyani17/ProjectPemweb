                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody class="align-middle">
                            ...
                            <tr>
                                ...
                                @if($criterion->kategori === "COST")
                                    <td class="text-center">{{ round($min, 3) }} / {{ round($siswa->alternative_value, 3) }} = {{ round($min/$siswa->alternative_value, 3) }}</td>
                                @elseif($criterion->kategori === "BENEFIT")
                                    <td class="text-center">{{ round($siswa->alternative_value, 3) }} / {{ round($max, 3) }} = {{ round($siswa->alternative_value/$max, 3) }}</td>
                                @endif
                                @if($criterion->kategori === "COST")
                                    <td class="text-center">{{ round($min/$siswa->alternative_value, 3) }} / {{ round($sumMin, 3) }} = {{ round(($min/$siswa->alternative_value)/$sumMin, 3) }}</td>
                                @elseif($criterion->kategori === "BENEFIT")
                                    <td class="text-center">{{ round($siswa->alternative_value/$max, 3) }} / {{ round($sumMax, 3) }} = {{ round(($siswa->alternative_value/$max)/$sumMax, 3) }}</td>
                                @endif
                            </tr>
                            @endforeach
                        </tbody>
    ...
    <div class="card mb-4">
        <div class="card-body table-responsive">
            ...
            <table id="datatablesSimple" class="table table-bordered">
                ...
                <tbody class="align-middle">
                    <?php
                    $siswas = \App\Models\Siswa::join('alternatives', 'siswa.id', '=', 'alternatives.siswa_id')
                        ->join('criterias', 'alternatives.criteria_id', '=', 'criterias.id')
                               ...
                               // $bobots    = \App\Models\PriorityValue::where('criteria_id', $idCriteria)->get();
                                ...
                                foreach ($bobots as $bobot) {
                                    ...
                                    if ($divider['kategori'] === 'BENEFIT' && $val != 0) {
                                       ...
                                       // $bobotValue = ($rowTotal/$criteria_analysis->priorityValues->count());
                                        $bobotValue =(($val/$maxVal)/$sumMaxVal)*$rowTotal/$criteria_analysis->priorityValues->count();
                                        //$bobotValue = $bobot->value;
